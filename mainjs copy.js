 ...
 *****************/
const elements = {
  searchLoading: document.getElementById("loader"),
...
class MyMap {
  constructor() {
    ...
    this.storeInfoPopup = new google.maps.InfoWindow({
      borderRadius: 0,
      arrowSize: 10,
      borderWidth: 1,
  ...
  store: {
    ...
    details: function (store) {
      ...
      if (address.trim()) {
        ...
        <p class="d-flex align-items-center">
          <i class="fas fa-car mx-2" style="color:${ICON_COLOR} !important;"></i>
          <a class="flex-grow-1" href="https://maps.google.com/maps?saddr=${myMap.center["lat"]},${myMap.center["lng"]}&daddr=${store["lat"]},${store["lng"]}" target="new" style="color:${ICON_COLOR} !important;">${address.replace(", ,", ", ")}</a>
        </p>
        `;
      }
      ...
      if (store["phone"]) {
        html += `
        <p class="d-flex align-items-center">
          <i class="fas fa-phone-alt mx-2" style="color:${ICON_COLOR} !important";></i>
          <a class="flex-grow-1" href="tel:${store["phone"]}" style="color:${ICON_COLOR} !important;">${store["phone"]}</a>
        </p>
        `;
      }
    ...
    availability: function (store) {
		if (COUNTRY=="unitedkingdom" && BRAND=="FodyFoods"){
			return `Fody stockist - please call to confirm availability or to request an order`;
		}else if (store["products"].length<2){
			return `Please contact the store to confirm availability`;
		}else{
      		return `${store["products"].length} Product(s) available at this store<br/> Please contact the store to confirm availability`;
		}
    },
    popup: function (store) {
      const address = `${store["address"]}<br>${store["city"]}, ${store["state"]}, ${store["zipcode"]}`;
      const distance = parseFloat(store["distance"]).toFixed(2);

      const html_phone = store["phone"]
        ? `<p class="my-1" style="font-size:1.3em;color:${ICON_COLOR} !important"><i class="fas fa-phone-alt"></i> ${store["phone"]}</p>`
        : "";

      // TODO: ES6 support required
      const html = `
        <div class="d-flex flex-column">
          <button type="button" class="btn btn-link" style="align-self: flex-end;font-size:1.2em;color:${ICON_COLOR} !important;" onclick="handle.popup.close();" >
            <i class="fas fa-arrow-left mr-1"></i> back to results
          </button>
          <div class="p-4">
            <h5 style="font-size:1.5em;color:${ICON_COLOR} !important">${store["name"]}</h5>
            <div  style="font-size:1.3em;color:${ICON_COLOR}; !important">${address.replace(",,", ",")}</div>
            ...
            <a class="btn btn-outline-primary btn-block my-1 iwindow_directions" href="https://maps.google.com/maps?saddr=${myMap.center["lat"]},${myMap.center["lng"]}&daddr=${store["lat"]},${store["lng"]}" target="new"  style="font-size:1.3em;color:${ICON_COLOR} !important;border-color:${ICON_COLOR} !important;"><i class="fas fa-car"></i> DIRECTIONS</a>
          </div>
        </div>
          `;
      ...
      <div class="list-group-item list-group-item-action" >
          <div class="d-flex w-100 justify-content-between align-items-center pointer"  onClick="handle.marker.click(${store["id"]});">
            <div class="flex-grow-1">
                ...
                <p class="my-1"  style="color:#495057 !important;">${address.replace(", ,", ",")}</p>
                <small>${distance} ${distancemeasure}</small>
            </div>
            <div class="d-flex">
              <a href="https://maps.google.com/maps?saddr=${myMap.center["lat"]},${myMap.center["lng"]}&daddr=${store["lat"]},${store["lng"]}" target="new" style="color:${ICON_COLOR};"><i class="fas fa-car mr-2"></i></a>
              <a href="javascript:void(0);" onclick="handle.marker.click(${store["id"]});" style="color:${ICON_COLOR};"><i class="fas fa-info-circle ml-2"></i></a>
            </div>
          </div>
...
const fetch = {
  _loading: function () {
	...
	show.mainbar.loading();
  },
  _done: function () {
    //hide.map.loading();
    //hide.sidebar.loading();
	hide.mainbar.loading();
  },
  products: function (options, callback) {
    this._loading();

    $.get(
      "https://shelfsmartdata.com/app/slt/shelfsmartlocator/include/mainproducts.php",
      options,
      (result, status) => {
        this._done();
...
const show = {
  ...
  mainbar: {
    loading: function () {
      $(elements.searchLoading).find(".loading").show();
    },
  },
  //sidebar: {
    //loading: function () {
...
const hide = {
  mainbar: {
    loading: function () {
      $(elements.searchLoading).find(".loading").hide();
    },
  },
  //sidebar: {
    //loading: function () {
...
// Fetch products
fetch.products({ brand: BRAND, country: COUNTRY, upcs: UPCS }, set.products);
...
// Show online stores if exists and adjust for online button
if (HAS_STORES) {
  $(elements.modalCol).show();
  $('#search_nearby').removeClass('col-md-4');
  $('#search_nearby').addClass('col-md-3');
  $('#search_input').removeClass('col-md-4');
  $('#search_input').addClass('col-md-3');
  $('#find_now').removeClass('col-md-4');
  $('#find_now').addClass('col-md-3');
}
// Set background-color if available
if (HAS_BGCOLOR) {
	$('body').css('background-color',bg_color);
	$('.list-group-item').css('background-color',bg_color + " !important");
